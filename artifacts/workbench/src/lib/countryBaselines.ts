// Static country baseline profiles for the Country Report builder.
//
// Country reports must NOT be built only from incident records in the
// current reporting window. Each report layers three data sources:
//
//   1. Current incidents      (7-day window, filtered upstream)
//   2. Country baseline        (this file — operating environment,
//                               security context, known risk areas,
//                               infrastructure / medical / evac /
//                               resource-sector exposure)
//   3. Location watchlist      (named cities, corridors, sectors that
//                               the report always reports against,
//                               regardless of whether incidents landed
//                               in the current window)
//   4. Lookback context        (30 / 90 day, computed at render time)
//
// The baseline is editorial reference content — it changes slowly, is
// curated, and should not drift on the back of a single quiet week.
// It is the "background operating picture" the user asked for when
// current-window reporting is thin.
//
// Lookup is case-insensitive on country name; falsy when no baseline
// is curated (the report degrades gracefully and shows only the live
// data layers).

export interface CountryBaseline {
  // One-paragraph framing of how the country actually runs day-to-day.
  // Should describe geography, jurisdiction, federal/provincial split,
  // and the rough cadence of activism / public-order activity. Not a
  // metric restatement.
  operatingEnvironment: string;
  // Standing security context: who is on the street, where security
  // forces concentrate, what cycles drive unrest, what political
  // calendar items routinely move risk.
  securityContext: string;
  // Named geographies (cities, provinces, regions) the report's brief
  // is expected to cover even when no incident landed in the current
  // window. These appear as bullets in the Country Baseline section.
  knownRiskAreas: string[];
  // Headline cities / provinces — short labels for the at-a-glance
  // bullet list (different from knownRiskAreas: those describe risk,
  // these describe geography).
  keyCitiesProvinces: string[];
  // Standing movement / access friction: roads, checkpoints, permit
  // regimes, internet shutdowns, curfews that recur.
  movementConstraints: string;
  // Infrastructure limits operationally relevant to staff and supply
  // chains (ports, airports, fuel, power, comms).
  infrastructureLimits: string;
  // Medical / casevac considerations: hospital quality, evac routes,
  // distance to tier-1 medical, weather/altitude constraints.
  medicalEvac: string;
  // Resource-sector exposure: who operates, what the assets are, how
  // they are protected, what the dispute history looks like.
  resourceSectorExposure: string;
  // Standing location watchlist — every entry is reported against the
  // current / 30 / 90 windows regardless of whether incidents
  // landed. Match tokens are case-insensitive substring matches
  // against incident.location (and incident.title as a fallback).
  // `label` is the display name; `match` is the list of strings to
  // check against location/title. Use multiple match strings for
  // places with common alternative spellings.
  locationWatchlist: Array<{
    label: string;
    note: string;
    match: string[];
  }>;
...
const PAPUA_NEW_GUINEA: CountryBaseline = {
  operatingEnvironment:
...
// Keyed by lowercase, trimmed country name. Common aliases mapped onto
// the same baseline (e.g. "West Papua" / "Indonesia; West Papua" all
// resolve to PAPUA).
const REGISTRY: Record<string, CountryBaseline> = {
  "papua": PAPUA,
  "west papua": PAPUA,
  "indonesian papua": PAPUA,
...
export function getCountryBaseline(name: string | null | undefined): CountryBaseline | null {
