...
function ScheduleTable({
  ...
  return (
    <div className="overflow-x-auto border border-border rounded-sm">
      <table className="min-w-[1250px] w-full text-[11px]">
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.id} className="border-t border-border align-top">
              ...
              <td className="px-2 py-2">{row.venue ?? "—"}</td>
              <td className="px-2 py-2">{row.eventType ?? "—"}</td>
              <td className="px-2 py-2">{row.issue ?? "—"}</td>
              <td className="px-2 py-2">{row.organiser ?? "—"}</td>
              <td className="px-2 py-2">{row.startTime ?? "—"}</td>
              <td className="px-2 py-2">{row.attendance == null ? "Not confirmed" : row.attendance}</td>
              <td className="px-2 py-2">{row.disruptionPotential ?? "—"}</td>
              <td className="px-2 py-2">{row.confidence}</td>
              <td className="px-2 py-2 whitespace-nowrap">{row.sourcePublishedAt?.slice(0, 10) ?? "—"}</td>
...
export default function ProtestScheduleEditor({
  ...
  const save = () => {
    ...
    if (!payload.country || !payload.sourceTitle || !payload.sourceUrl) return;
    const options = { onSuccess: () => { setEditing(null); setAdding(false); refresh(); } };
  ...
  const editor = (editing || adding) && (
    <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-2 border border-border rounded-sm p-3">
      {([
        ["eventDate", "Date"], ["country", "Country"], ["city", "City"], ["venue", "Venue"],
        ["eventType", "Event type"], ["issue", "Issue"], ["organiser", "Organiser"], ["startTime", "Start time"],
        ["attendance", "Attendance"], ["disruptionPotential", "Disruption potential"], ["sourcePublishedAt", "Source publication date"],
        ["sourceTitle", "Source title"], ["sourceUrl", "Source link"],
      ] as const).map(([key, label]) => (
        <label key={key} className="text-[10px] uppercase tracking-wide text-muted-foreground">
          {label}
          <Input type={key === "eventDate" || key === "sourcePublishedAt" ? "date" : key === "attendance" ? "number" : "text"} value={draft[key]} onChange={(e) => set(key, e.target.value)} className="h-8 mt-1 text-xs normal-case tracking-normal" />
        </label>
      ))}
      <label className="text-[10px] uppercase tracking-wide text-muted-foreground">Confidence<select value={draft.confidence} onChange={(e) => set("confidence", e.target.value)} className="h-8 mt-1 w-full border rounded-sm text-xs"><option>High</option><option>Moderate</option><option>Low</option></select></label>
      ...
      <div className="col-span-full flex gap-2 justify-end"><Button type="button" variant="outline" className="h-8 text-xs" onClick={() => { setEditing(null); setAdding(false); }}>Cancel</Button><Button type="button" className="h-8 text-xs" disabled={create.isPending || update.isPending} onClick={save}>{create.isPending || update.isPending ? "Saving…" : "Save event"}</Button></div>
    </div>
  );
  return (
    <section className="border border-border rounded-sm p-4 mb-4" data-testid="protest-schedule-editor">
      ...
      {!model.searchCompleted && rows === 0 ? (
        ...
        <>
          ...
          <ScheduleTable rows={model.schedule} onEdit={(row) => { setEditing(row); setAdding(false); setDraft(draftOf(row)); }} />
        </>
      )}
      {model.watchlist.length > 0 && <><h4 className="font-semibold text-xs mt-4 mb-2">{PROTEST_WATCHLIST_HEADING}</h4><ScheduleTable rows={model.watchlist} onEdit={(row) => { setEditing(row); setAdding(false); setDraft(draftOf(row)); }} /></>}
