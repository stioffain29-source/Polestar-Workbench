...
import {
  ...
  TRIAL_EXPOSURE_ACCENT,
  TRIAL_EXPOSURE_FILL,
  TRIAL_EXPOSURE_LABEL,
  TRIAL_EXPOSURE_ORDER,
} from "@/lib/jakartaTrialMap";
...
export interface JakartaTrialMapProps {
  incidents: CountryFastFactsIncident[];
  issueDate?: string;
}

/**
 * TRIAL Jakarta city movement-posture map (Task #290). SEPARATE from the live
 * Jakarta city report and JakartaCorridorMap — a throwaway design surface for a
 * QUIET redesign:
 *
 *   • seven exposure-shaded operating zones (1–7, fixed task order), each
 *     washed by its OWN live rating (High / Elevated / Monitored / Low / Not
 *     assessed) — 1:1 with the numbered pins and the panel;
 ...
 *
 * PDF parity mirrors the live map: crossOrigin <img> tiles + a single
 * offscreen-canvas → data-URL <img> overlay, both html2canvas-safe.
 */
export default function JakartaTrialMap({
  incidents,
  issueDate,
}: JakartaTrialMapProps) {
  const model = useMemo(() => buildJakartaTrialModel(incidents), [incidents]);
  ...
  useEffect(() => {
    ...
    const drawBlob = (
      ...
      ctx.closePath();
      const fill = hexToRgb(TRIAL_EXPOSURE_FILL[rating]);
      ctx.fillStyle = `rgba(${fill.r},${fill.g},${fill.b},${BLOB_ALPHA[rating]})`;
      ctx.fill();
      const acc = hexToRgb(TRIAL_EXPOSURE_ACCENT[rating]);
      ctx.lineJoin = "round";
      ctx.lineWidth = 1.2;
    ...
    const drawPin = (
      ...
      ctx.fillStyle = TRIAL_EXPOSURE_ACCENT[rating];
      ctx.fill();
      ctx.lineWidth = 2;
    ...
    const draw = () => {
      ...
      const shadedZones = model.zones
        ...
        .sort(
          (a, b) =>
            TRIAL_EXPOSURE_ORDER.indexOf(b.zone.rating) -
            TRIAL_EXPOSURE_ORDER.indexOf(a.zone.rating),
        );
      ...
      for (const c of JAKARTA_TRIAL_CORRIDORS) {
              ...
              <span key={lvl} style={legendRowStyle}>
                <span
                  style={{
                    ...
                    width: 12,
                    height: 12,
                    background: TRIAL_EXPOSURE_ACCENT[lvl],
                    border: `1px solid ${NAVY}`,
                  }}
                />
                <span style={legendTextStyle}>{TRIAL_EXPOSURE_LABEL[lvl]}</span>
              </span>
            ))}
              ...
              >
                <span
                  style={{
                    ...
                    height: 18,
                    borderRadius: 9,
                    background: TRIAL_EXPOSURE_ACCENT[z.rating],
                    color: "#ffffff",
                    fontFamily: "'Roboto Condensed', Roboto, sans-serif",
                    fontSize: 11,
                    fontWeight: 700,
