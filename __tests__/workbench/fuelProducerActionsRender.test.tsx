...
// Rendered-markup verification for the Fuel Watch "Producer and Buyer
// Actions" table: rows built by the REAL builder from a realistic crisis
// week (mirroring live prod titles) must render in the REAL table
// component — buyer supplier-pivot, refiner-margin and refinery-fire
// signals included, syndicated pivot rewrites collapsed to one row.
...
describe("ProducerActionsTable rendered markup", () => {
  ...
  it("renders the buyer supplier-pivot row once (syndicated copy collapsed)", () => {
  ...
  it("renders the refiner-margin and refinery-fire market signals", () => {
    expect(html).toContain("US refiner margins hit new records");
    expect(html).toContain("Oil refinery ablaze in Cuba");
    expect(html).toContain("Market / supply signal");
  });

  it("renders the cross-read routing action with all four columns", () => {
    expect(html).toContain("Two Saudi Oil Tankers Reroute");
