...
describe("flashpoint report consistency", () => {
  ...
  test("watch next labels forecast items as upcoming/unconfirmed and never calls one the most serious reported incident", () => {
    ...
    expect(lines.some((l) => /Pakistan/i.test(l))).toBe(true);
  ...
  test("dateless forecast rows stay out of the confirmed table and appear only in Watch Next", () => {
    ...
    expect(ds.autoWatchNext).toMatch(/Thailand/i);
    expect(ds.autoWatchNext).not.toMatch(/date confirmed/i);
  });
  ...
  test("activism and unrest reads name each table's own lead", () => {
    const rows = [
      inc({
  ...
  test("forecast reconciles unconfirmed Watch Next signals when no confirmed table exists", () => {
    ...
    expect(ds.autoWatchNext).toMatch(/Malaysia/i);
    expect(ds.autoWatchNext).not.toMatch(/date confirmed/i);
  });
  ...
  test("process and commentary rows stay out unless a live public-order event is evidenced", () => {
    const rows = [
      ...
      inc({
        ...
        summary: "Gen Z protest relatives said they will mark a commemoration day on the calendar.",
        country: "Nepal",
        location: "Kathmandu",
        severity: "extreme",
      }),
