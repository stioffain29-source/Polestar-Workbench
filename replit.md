...
- Fuel-market PRICES are now LIVE: `runMarketPricesIngest` (`@workspace/ingest`, `lib/ingest/src/marketPrices.ts`) pulls Brent (FRED DCOILBRENTEU), WTI (DCOILWTICO) and jet fuel (DJFUELUSGULF) from FRED's public `fredgraph.csv` endpoint (no API key) and writes the canonical `hardNumbers` shape into every `topic='fuel'` report, anchored to each report's issue date (latest observation ≤ issue date; 7-day change; ~6-point weekly jet trajectory). It runs inside `runIngestOnce` (after flashpoint+cargo) so the scheduler + admin route + `scrape:prod` all refresh prices, and is wrapped in its own try/catch so a FRED outage cannot fail the incident ingest. CLI: `pnpm --filter @workspace/scripts run scrape:prices [--commit]`. This REPLACED the hardcoded `FUEL_MARKET_DATA_SAMPLE` fabricated prices (Brent 109.26 / WTI 101.02 / jet 4.152) that never changed — the root cause of the "prices not changing / you are telling me lies" complaint. The editor no longer auto-seeds that sample into the preview/PDF (it only loads via the explicit "Load sample" button); a report with no saved data renders empty market fields rather than fake numbers.
- Only `flashpoint` + `cargo_watch` have INCIDENT scrapers. `energy`/`fertiliser`/`shipping`/`strikes` incident layers are STATIC / IMPORT ONLY — no live source exists to ingest; only `fuel` additionally has the live FRED price feed above. Those report families are purely incident-driven (no `hardNumbers` market tiles), so they had no fabricated price data to fix. Cargo Map "0 geocoded" is a separate gap: ingest inserts incidents with null lat/long (no geocoding step).
