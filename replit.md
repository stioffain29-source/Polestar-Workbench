...
- **AIS vessel tracking** — `AIS_API_KEY` (+ `AIS_PROVIDER`=aisstream, `AIS_ENABLED`, `AIS_COLLECT_SECONDS`). Live ship-movement CONTEXT into the isolated `maritime_movement` table; never touches incidents. Windward is scaffolded only (no ingest).
- **Vessel registry** — `VESSEL_REGISTRY_API_KEY` (+ `VESSEL_REGISTRY_PROVIDER`=datalastic, `_ENABLED`, `_API_BASE`, `_MAX_LOOKUPS`). PAID. Additive bulk/container/LNG split over the AIS sample; unmatched vessels left unclassified.
