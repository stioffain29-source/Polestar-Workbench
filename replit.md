...
- NO follow-up task suggestions. Owner explicitly said "stop suggested tasks" — do not propose follow-up tasks after finishing work; just complete the requested work.
- NOTHING SHIPS WITHOUT BEING CHECKED 100%. Owner's explicit standing order. Every user-facing change MUST be verified against its RENDERED output before it is reported done or published — not just typecheck/font gates. The app is owner-gated (no live screenshot), so verify via rendered-markup tests (renderToStaticMarkup), headless PDF export inspection, or a mockup-sandbox visual render of the changed component. Layout/alignment counts: chips, buttons, spacing, centring.
