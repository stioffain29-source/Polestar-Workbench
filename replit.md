...
- Only `flashpoint` + `cargo_watch` have scrapers. `fuel`/`energy`/`fertiliser`/`shipping`/`strikes` are STATIC / IMPORT ONLY — no live source exists to ingest. Cargo Map "0 geocoded" is a separate gap: ingest inserts incidents with null lat/long (no geocoding step).
