import {
  ...
  OSM_DC_COUNTRIES,
  type OsmCountryResult,
} from "@workspace/ingest";
import { pool } from "@workspace/db";

/**
 * OpenStreetMap → Data Centre facility REGISTRY importer (supervised CLI).
 *
 * Populates the analyst-maintained `data_centre_facilities` table from public
 * OpenStreetMap data (Overpass API — free, no key). All logic lives in
 * @workspace/ingest (runOsmFacilityRegistryImport); this is a thin CLI wrapper
 * that prints a dry-run preview and closes the pool.
 *
 * STRICT no-fabrication: only facilities with a real OSM name AND coordinates
 * are imported; operator/capacity/status/risk are NEVER guessed (blank reads
 * "not reported"). The canonical OSM URL is stored on each row for provenance
 ...
 * it is NOT wired into the api-server scheduler.
 *
 * Usage:
 *   # dry-run everything (default — writes nothing):
 *   pnpm --filter @workspace/scripts run import:osm-data-centres
 *   # dry-run one country (Singapore first):
 *   pnpm --filter @workspace/scripts run import:osm-data-centres -- --country=SG
 *   # commit one country after reviewing the dry-run:
 *   pnpm --filter @workspace/scripts run import:osm-data-centres -- --country=SG --commit
 *   # scope to several / cap for sampling:
 *   pnpm --filter @workspace/scripts run import:osm-data-centres -- --country=SG,MY --limit=20
 */
...
function printPreview(r: OsmCountryResult): void {
...
async function main(): Promise<void> {
  ...
  if (countries) {
    const known = new Set(
      OSM_DC_COUNTRIES.flatMap((c) => [c.iso.toLowerCase(), c.country.toLowerCase()]),
    );
    const unknown = countries.filter((c) => !known.has(c.toLowerCase()));
    if (unknown.length > 0) {
      console.error(
        `Unrecognised --country value(s): ${unknown.join(", ")}\n` +
          `Valid: ${OSM_DC_COUNTRIES.map((c) => `${c.iso} (${c.country})`).join(", ")}`,
      );
      await pool.end();
      process.exit(1);
    }
  ...
  const summary = await runOsmFacilityRegistryImport({
    commit,
    countries,
    perCountryLimit,
  });
