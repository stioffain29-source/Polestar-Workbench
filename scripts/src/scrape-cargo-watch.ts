...
// "pilferage" / "pilfered" are deliberately excluded as bare terms — they catch
// power/coal/resource theft. Use cargo-qualified variants below instead.
const ALLOW = [
  "cargo theft",
  "cargo hijack",
  "cargo crime",
...
// Word-boundary match so "india" doesn't match inside "Indian-origin".
...
async function main(): Promise<void> {
  ...
  for (const f of FEEDS) {
    ...
    const err = feedErrors.find((e) => e.feed === f.label);
    if (err) {
      console.log(`  ${f.label.padEnd(28)} ERROR: ${err.error}`);
    } else {
      console.log(`  ${f.label.padEnd(28)} found=${s.found.toString().padStart(3)}  accepted=${s.accepted.toString().padStart(3)}  rejected=${s.rejected.toString().padStart(3)}`);
    }
  }
  ...
  console.log("\n=== Totals ===");
  console.log(`  Feeds queried        : ${FEEDS.length}`);
  console.log(`  Feed errors          : ${feedErrors.length}`);
  console.log(`  Items found          : ${accepted.length + rejected.length}`);
  ...
  const rejSample = rejected.slice(0, 8);
  for (const r of rejSample) {
    console.log(`  · [${r.reason}] (${r.feedLabel}) ${r.title}`);
  }
  ...
  const after = await db.execute(sql`SELECT COUNT(*)::int AS count FROM incidents WHERE topic='cargo_watch'`);
  const count = (after.rows[0] as { count: number } | undefined)?.count ?? 0;
  console.log(`\nInserted ${rows.length} rows. cargo_watch total now: ${count}`);
