import { runPngExtractBackfill } from "@workspace/ingest";
...
//
//   pnpm --filter @workspace/scripts run backfill:png-extract            # dry-run
//   pnpm --filter @workspace/scripts run backfill:png-extract -- --commit
//   ... -- --commit --limit=500
...
async function main(): Promise<void> {
  ...
  const summary = await runPngExtractBackfill({ commit, limit });
