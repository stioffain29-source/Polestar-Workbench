...
- A **standalone `<canvas>`** element placed in the Leaflet overlay is ALSO silently dropped by html2canvas (basemap rasterises, marker vanishes) — verified empirically on the `CountryReportMap` PDF export. So "no canvas" means no canvas AT ALL in the overlay, not just Leaflet's panes. Draw count badges as centred text in the `<div>` (flex-centre + ~2px `paddingBottom` to offset html2canvas's low-text rendering), never on a `<canvas>`.
