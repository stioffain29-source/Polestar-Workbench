...
- A fail-safe publication model must remain analytical: canonical geography, severity and validated schedules can support substantive count-free prose. Never let recovery emit internal phrases such as “accepted record” or an empty Watch Next.
  **Why:** Flashpoint’s primary and canonical models both failed validation, silently exposing emergency boilerplate despite a substantive canonical dataset.
  **How to apply:** log both validation-error sets, keep the hard gate, and make the final recovery model name only canonical places/severity while always supplying grounded monitoring indicators.
