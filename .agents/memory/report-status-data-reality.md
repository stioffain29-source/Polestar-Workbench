...
- Live-feed weekly drafts (especially Flashpoint/protests) must advance to today's issue date even when the newest accepted incident is older. Never clamp their reporting period back to the latest incident date; that revives old windows and old rows. Static/import-only topics may still clamp.
...
**Why:** A publication-calendar feature that keyed off `status='published'` or `country_slug` would have shipped blank. Separately, clamping a live Flashpoint draft to the latest incident made a 20 September report render the 8–14 September window and retain a 9 September item.

**How to apply:** Anchor publication and recency on `issueDate` (not status). Keep live-feed drafts current even in a quiet week; let the empty/quiet current window render honestly. Source country/region from country/spot reports unless the data model changes.
