...
APAC severity is reassessed from the underlying facts at report time; inherited labels, dramatic wording and source volume do not control it. Consequences, intent, method, target, casualties and operating effects do. Bombing or armed attacks against defence/security actors classify before transport words, so vehicle references cannot turn them into operational disruption.
