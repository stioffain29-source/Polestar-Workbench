...
**How to apply:** Verify actual event/policy dates BEFORE curation. Preserve an explicit event/policy date; otherwise the later occurrence-timestamp rule below applies, labelled as a reported evidence date rather than pretending it is the event date. Never substitute publication, scrape, ingestion or report dates; exclude unresolved DATE UNVERIFIED items. Curate to the requested region before canonical assembly, require at least five selected developments, and pass all selected coordinate/fallback points to preview and PDF renderers. Build once, persist the complete canonical object, validate it after reload and fail closed when absent or stale. Event-family keys must describe the actual event, never just a country: same-country events such as a capital missile attack and a pipeline shutdown remain separate, while variant headlines for one cross-border incursion collapse together.

Regional Weekly maps must appear in both preview and the actual Download PDF path, and must use keyed CARTO Positron rather than simplified hand-drawn country outlines. This supersedes both the earlier no-map direction and the outline treatment.

**Why:** An old drawing function still existed but was never called by the active PDF branch; its presence in source was not proof of export parity. Effect-driven map libraries also cannot populate an immediately captured static HTML snapshot. On 2026-09-21 the owner explicitly rejected the outline map treatment and required a return to CARTO Positron.

**How to apply:** Reuse the Workbench CARTO provider for both regional previews and PDFs, through the same static-renderable map in both surfaces. Preserve saved evidence, marker order, severity and captions; do not redesign the report or replace the basemap with an outline approximation. Separate overlapping numbered badges without changing geographic anchors or saved points. Await remote basemap tiles under a bounded deadline and stop export on tile failure rather than shipping blank geography. Verify the actual browser-generated PDF visually, not just extracted text.
