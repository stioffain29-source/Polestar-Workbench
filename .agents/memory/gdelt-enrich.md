...
**Match rate is structurally LOW, not a bug.** GDELT's APAC Demonstrations coverage is sparse and UNEVEN: Japan and Malaysia (our two highest-volume countries) returned ZERO Demonstrations events; India/Indonesia/Philippines had rich data. A budget-capped run picks countries stalest-first, so a tiny cap can burn the whole budget on GDELT coverage gaps. When validating, use a full cap (10) so it reaches covered countries. Expect single-digit matches per run — the value is precision where GDELT covers, falling back gracefully everywhere else. The TOTAL "Incidents matched" is capped UPSTREAM by GDELT's event pool (≤10 countries × QU cap), so improving our side of the match (e.g. resolving redirect URLs) moves "via source URL" but barely moves the total — do not expect a big total jump from any of our matching improvements.

**Google News redirect resolution feeds the URL match — see [google-news-url-resolution.md](google-news-url-resolution.md).** gdeltEnrich now matches on `normalizeUrl(resolvedUrl ?? sourceUrl)`; the URL branch can only fire once redirects are resolved into `incidents.resolved_url`.
