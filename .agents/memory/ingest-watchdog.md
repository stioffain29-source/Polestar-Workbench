...
**Map-critical ordering:** run the direct APAC/Indonesia incident collectors
immediately after strikes and before every ancillary context feed; run the
remaining narrower topics before Flashpoint. Flashpoint has the broadest feed
set and may consume most of a worker run.
...
direct local feeds ahead of protest schedules, maritime sources and title
translation, and keep Flashpoint as the last incident collector rather than
widening the map window.
