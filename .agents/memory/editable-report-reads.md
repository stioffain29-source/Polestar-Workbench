...
## Topic section overrides (fast facts, panel reads, market prices, fuel bullets/rows)
...
- Gotcha: override keys are exact-label matches, so any change to a builder's tile label orphans saved overrides. Orphans are now SURFACED (not silently dropped): the editor's `OrphanedFastFactsPanel` lists override keys not matching any current auto tile, with re-attach (moves the entry; target's existing non-blank fields win the merge) and clear actions via `orphanedFastFactOverrideKeys` / `reattachFastFactOverride` / `clearFastFactOverride` in `topicSectionOverrides.ts`. Nothing is migrated silently; orphaned entries stay inert in the render path.
- Extension: fuel Gulf/Hormuz BULLET overrides (`gulfBulletOverrides`, keyed by the AUTO bullet line, `{text?, suppressed?}`) and Market and Operator Responses ROW overrides (`marketOperatorOverrides`, keyed `date|actor|action` via `marketOperatorRowKey`, per-cell fields + `suppressed`). Same one-authority pattern: `applyGulfBulletOverrides` / `applyMarketOperatorOverrides` called in BOTH `ReportPreview` and `exportTopicReportPdf`; suppressing every row hides the whole table section; the editor lists bullets/rows from the same `buildFuelWatchReportData` build. Same exact-key orphaning gotcha as fast-fact labels.
