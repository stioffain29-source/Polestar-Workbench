...
const PACIFIC_CRIME: RegExp =
  /\b(armed robbery|robbery|robbed|hold[- ]?up|carjack(?:ing|ed)?|home invasion|stabb(?:ed|ing)|machete attack|bush[- ]?knife|raskol|tribal (?:fight|clash|war|warfare|violence|conflict)|gang[- ]?(?:rape|violence|attack|war|fight|members?|shooting)|police raid|raid(?:ed|s)?|wanted (?:criminal|man|men|suspect|fugitive|offender)|shot dead|shooting|gunned down|opened fire|gun(?:point|fire|fight)|gun battle|firefight|kidnap(?:p?ed|ping)?|abduct(?:ion|ed)?|looting|murder(?:ed|s)?|manslaughter|kill(?:ed|ings?|s)?|fatalit(?:y|ies)|massacre|found dead|beaten to death|mob (?:attack|violence|justice|turns|sets|storms|burn|beat)|payback (?:killing|attack)|sorcery|riot(?:ing|s)?|arson|rebels?|separatists?|insurgen(?:t|ts|cy)|deadly clash|armed clash|violent clash|ambush(?:ed|es|ing)?|landmine|land mine|detained by (?:security|police|the military|military|marines?|tni|soldiers?|joint|task ?force)|opm|tpnpb|armed criminal group|criminal armed group|rascal gang|vehicle theft|car theft|stolen (?:vehicle|car|truck|motor vehicle)|firearm|assault(?:ed|s)?|settlement (?:fight|clash|brawl|violence|raid|war)|roadblock|road block|recover(?:ed|s)? (?:the )?bodies|bodies of (?:\d|the ))\b/i;
