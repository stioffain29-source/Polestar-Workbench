...
const FLASHPOINT_TITLE_HARD_EXCLUDE: RegExp[] = [
  ...
  // Anti-corruption / integrity enforcement on business figures — "Nepal's
  // crackdown on businessmen", CIAA probes, graft raids on traders. Law-
  // enforcement and economic-policy copy, not a protest or public-order event.
  /\b(crackdown|clampdown|cracks? down|clamps? down|sweep|sweeps|drive)\b[^.!?]{0,50}\b(businessmen|businessman|business leaders?|business community|entrepreneurs?|industrialists?|tycoons?|magnates?|traders?|industrialists?)\b/i,
  /\b(anti[- ]?corruption|anti[- ]?graft|integrity (?:unit|commission|drive|sweep)|ciaa|ac(?:b| commission))\b[^.!?]{0,50}\b(businessmen|businessman|business leaders?|entrepreneurs?|traders?|raid|raids|arrest|arrests|detain|detention|charge|charges|probe|investigation)\b/i,
  // Firearms regulation / struggling gun-retail policy copy — not a
...
const CONFLICT_HARD_EXCLUDE: RegExp[] = [
  /\b(?:acquit\w*|convict\w*|sentenc\w*|indict\w*|charged?|trial|pleads? guilty|found guilty)\b[^.]{0,55}\b(?:federal )?(?:murder|homicide|shooting|firearms?|weapons?) (?:charge|charges|case|trial)\b/i,
